...
export default function createStateContext<T>(
  reducer: Reducer<T>,
  initialState: T,
  middleware: StateMiddleware | StateMiddleware[] = [],
): IStateContext<T> {
  const middlewares = Array.isArray(middleware) ? middleware : [middleware];
  const StateContext = createContext<ContextState<T>>([
    initialState,
    () => null,
  ]);
